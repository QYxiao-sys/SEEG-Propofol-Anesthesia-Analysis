function out = get_klmi_jia(lf_phase,hf_env,nbins,nsurr,randType)
% out = get_klmi_jia(lf_phase,hf_env,nbins) OR
% out = get_klmi_jia(lf_phase,hf_env,nbins,nsurr,randType)
%
% This function calls "calc_klmi" to calculate MI values 
% Inputs: 
...
%       preserves the power spectrum of the signal.（有bug ?）
...
%
% Outputs:
% - out: structure with six fields
%        MI: array of MI values, of size=[N,M,# of realizations]
%        MIp: associated p-valuel,
%        MIsurr_avg: average MI across surroagte MI values 
%        MIsurr_std: standard deviation across surroagte MI values
...
%subsequently called
if ndims(lf_phase) > 2; highdim = true; else highdim = false; end 
...
% calculate observed KL-MI
disp('calcualting observed KL-MI...')
...
out.MI = calc_klmi(centers, mean_amps, uniform);
...
if doSurr
    ...
    for s=1:nsurr
        ...
        surr_mi(st:fn) = calc_klmi(centers, surr_amps, uniform); %update last
    end
    ...
    out.MIsurr_avg    = mean(surr_mi,dimsurr); 
    out.MIsurr_std    = std(surr_mi,0,dimsurr); 
end
...
disp('...finished KL-MI for one channel pair')